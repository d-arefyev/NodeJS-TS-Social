...
const SubMenuSearch: React.FC<SubMenuSearchProps> = ({ onClose }) => {
  ...
  useEffect(() => {
    if (allUsers.length === 0) {
      getAllUsers(); // Загружаем всех пользователей, если они еще не загружены
    }
  }, [allUsers, getAllUsers]);

  // Фильтрация пользователей на основе ввода
  useEffect(() => {
    searchUsers(query); // Воспользуемся функцией поиска из хука
  }, [query, searchUsers]);
  ...
  const handleUserClick = () => {
    onClose(); // Закрыть сабменю при клике на пользователя
  };
...
// const SubMenuSearch: React.FC<SubMenuSearchProps> = ({ onClose }) => {
//   const { allUsers, isLoading, error, getAllUsers } = useUser();
//   const [query, setQuery] = useState<string>(""); // Состояние для ввода
...
//       );
//     } else {
//       setFilteredUsers(allUsers); // Если поле поиска пустое, показываем всех пользователей
//     }
//   }, [query, allUsers]);
...
//                       />
//                     </div>
//                     {/* Обновляем ссылку на правильную страницу профиля другого пользователя */}
//                     <Link href={`/other-profile/${user._id}`}>
//                       <span className="hover:text-color-dark hover:font-semibold transition-all">
//                         {user.username}
//                       </span>
//                     </Link>
